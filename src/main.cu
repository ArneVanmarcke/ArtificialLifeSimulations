...
void initBoids(std::vector<Boid>& boids, int screenWidth, int screenHeight) {
    srand(time(NULL));

    for (int i = 0; i < NUM_BOIDS; i++) {
        float posX = randomFloat() * screenWidth;
        float posY = randomFloat() * screenHeight;
        
        float velX = (randomFloat() * 2.0f - 1.0f) * MAX_SPEED;
        float velY = (randomFloat() * 2.0f - 1.0f) * MAX_SPEED;
        
        boids[i] = Boid(Vector2(posX, posY), Vector2(velX, velY));
        boids[i].color = getSpeedColor(boids[i].velocity.magnitude(), MAX_SPEED);
    }
}

int getNeighbours(const std::vector<Boid>& boids, int self_idx, Boid* neighbours, float range) {
    int count = 0;
...
Vector2 calculateAlignment(Boid* boid, Boid* neighbours, int neighbour_count, float max_speed){
...
Vector2 calculateCohesion(Boid* boid, Boid* neighbours, int neighbour_count, float max_speed){
...
Vector2 calculateSeparation(Boid* boid, Boid* neighbours, int neighbour_count, float separationThreshold, float max_speed){
...
Vector2 calculateAttraction(Boid* boid, const Vector2* target, float max_speed){
    if(target == nullptr){
        return Vector2(0,0);
    }
...
Vector2 calculateExplosion(Boid* boid, Boid* neighbours, int neighbour_count) {
...
void SimulateBoids(std::vector<Boid>& boids, std::vector<Boid>& new_boids,
                   const Vector2* target, int screenWidth, int screenHeight, float dt, bool& explode) 
{
    for (int idx = 0; idx < NUM_BOIDS; idx++) {
        ...
        new_boids[idx] = Boid(new_position, new_velocity);
        new_boids[idx].color = getSpeedColor(new_velocity.magnitude(), MAX_SPEED);
    }
...
int main() {
    ...
    initBoids(boids, screenWidth, screenHeight);
    ...
    while (running && totalFrames < 10000) {
        ...
        SimulateBoids(boids, new_boids, target, screenWidth, screenHeight, dt, explode);
